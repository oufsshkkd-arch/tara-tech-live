...
// Meta / Facebook mini-mark — pairs with TT badge on dual-tracked cards
function MetaMark({ size = 12 }: { size?: number }) {
  return (
    <svg viewBox="0 0 24 24" width={size} height={size} aria-hidden fill="currentColor">
      <path d="M24 12.073C24 5.405 18.627 0 12 0S0 5.405 0 12.073C0 18.1 4.388 23.094 10.125 24v-8.437H7.078v-3.49h3.047V9.412c0-3.007 1.792-4.668 4.533-4.668 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.875v2.25h3.328l-.532 3.49h-2.796V24C19.612 23.094 24 18.1 24 12.073z" />
    </svg>
  );
}
...
export default function AdminInsights() {
  ...
  const [fbInput, setFbInput] = useState(brand.facebookPixelId ?? "");
  ...
  function saveFbId() {
    setBrand({ facebookPixelId: fbInput.trim() || undefined });
    setFbSaved(true);
  ...
  return (
    <div className="space-y-6">
      ...
      <Section
        ...
        description="Pixel ID ديال Meta للـ Standard Events (ViewContent, AddToCart, Purchase) و conversion tracking."
      >
        <Field
          label="Facebook Pixel ID"
          hint='كتلقاه فـ Meta Events Manager → Data sources → Web Pixel → Settings (مثال: "4325181661059444")'
        >
          <div className="flex gap-2">
            <input
              className="input flex-1 font-mono"
              placeholder="4325181661059444"
              value={fbInput}
            ...
            <button
              ...
              style={fbSaved ? undefined : { backgroundColor: META_BLUE }}
            >
              {fbSaved ? "✓ Saved" : "Activate"}
            </button>
          </div>
        </Field>

        {brand.facebookPixelId ? (
          <div
            className="flex items-center gap-3 rounded-xl px-4 py-3"
            style={{ backgroundColor: `${META_BLUE}10`, border: `1px solid ${META_BLUE}40` }}
          >
            <CheckCircle2 className="h-4 w-4 text-green-600 shrink-0" />
            <span className="text-sm" style={{ color: META_BLUE }}>
              <span className="font-bold">Facebook Pixel مفعل</span> — ID: <span className="font-mono font-semibold">{brand.facebookPixelId}</span>
            </span>
            <span className="ms-auto" style={{ color: META_BLUE }}>
              <MetaMark size={18} />
            </span>
          </div>
        ) : (
          <div className="flex items-center gap-2 rounded-xl bg-amber-50 border border-amber-200 px-4 py-3">
            <Info className="h-4 w-4 text-amber-600 shrink-0" />
            <span className="text-sm text-amber-700">
              Facebook Pixel غير مفعّل — أدخل Pixel ID باش تبدا تتبع conversions.
            </span>
          </div>
        )}
        ...
        <div className="mt-4 grid sm:grid-cols-3 gap-3">
          {[
            { event: "ViewContent", label: "صفحة منتج",   icon: Eye },
            { event: "AddToCart",   label: "بدء النموذج", icon: ClipboardList },
            { event: "Purchase",    label: "طلب مؤكد",     icon: CheckCircle2 },
          ].map(({ event, label, icon: Icon }) => (
            <div
              key={event}
              className="flex items-center gap-2.5 rounded-xl border border-line bg-white px-3 py-2.5"
            >
              ...
              <Activity className="h-3 w-3 ms-auto shrink-0" style={{ color: META_BLUE }} />
            </div>
          ))}
        </div>
      </Section>
